...
function initThree() {
    ...
    var sceneTexture = new THREE.TextureLoader().load('img/mobile/page/map_bg.png');
...
function updateScreenPosition() {
    ...
    var zooPos = zoos[0].position;
    var labelZoo_pos = createPosition(zooPos,31.5);
    var showZoo = updateAnnotationOpacity(labelZoo_pos);
    var vectorZoo = new THREE.Vector3(labelZoo_pos.x, labelZoo_pos.y, labelZoo_pos.z);

    vectorZoo.project(camera);
    // vector.x = Math.round((0.5 + vector.x / 2) * (canvas.width / window.devicePixelRatio));
    // vector.y = Math.round((0.5 - vector.y / 2) * (canvas.height / window.devicePixelRatio));

    vectorZoo.x = (0.5 + vectorZoo.x / 2) * (canvas.width / window.devicePixelRatio);
    vectorZoo.y = (0.5 - vectorZoo.y / 2) * (canvas.height / window.devicePixelRatio);

    var labelZoo = $(".zoo")[0];
    labelZoo.style.top = vectorZoo.y  + "px"; //80
    labelZoo.style.left = vectorZoo.x  + "px";
    labelZoo.style.display = showZoo ? "none" : "block";
